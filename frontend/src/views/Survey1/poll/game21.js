...
function CombinedGame() {
    ...
    const handleSubmit = async (event) => {
        ...
        try {
            const responses = await Promise.all([
                fetch('https://student-judgementgamewebdemo.onrender.com/submit', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
    ...
    const getResult = async () => {
        try {
            const response = await fetch("https://student-judgementgamewebdemo.onrender.com/find", { method: 'POST' });
